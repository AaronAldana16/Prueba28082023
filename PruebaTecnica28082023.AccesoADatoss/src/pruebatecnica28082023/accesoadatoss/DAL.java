 ...
 */
package pruebatecnica28082023.accesoadatoss;
...
public class DAL {
    ...
    static String obtenerCampos() {
        return "id,nombre,apellido,email,cargo";
    ...
    private static String obtenerSelect(Empleado pEmpleado) {
        ...
        sql += (obtenerCampos() + "from Empleados r"); // Agregar los campos de la tabla de Rol mas el FROM a la tabla Rol con su alias "r"
        return sql;
    }
    ...
    public static int crear(Empleado pEmpleado) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            sql = "INSERT INTO Empleado(Name,LastName,Email,UserName,Password) VALUES(?,?,?,?,?)"; // Definir la consulta INSERT a la tabla de Rol utilizando el simbolo ? para enviar parametros
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setString(1, pEmpleado.getNombre()); // Agregar el parametro a la consulta donde estan el simbolo ? #1  
                ps.setString(2, pEmpleado.getApellido()); 
                ps.setString(3, pEmpleado.getEmail());
    ...
    public static int eliminar(Empleado pEmpleado) throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                ps.setInt(1, pEmpleado.getId()); // Agregar el parametro a la consulta donde estan el simbolo ? #1 
    ...
    static int asignarDatosResultSet(Empleado pEmpleado, ResultSet pResultSet, int pIndex) throws Exception {
        ...
        pIndex++;
        pEmpleado.setId(pResultSet.getInt(pIndex)); // index 1
        pIndex++;
        pEmpleado.setNombre(pResultSet.getString(pIndex)); // index 2
        pIndex++;
        pEmpleado.setApellido(pResultSet.getString(pIndex)); // index 3
        pIndex++;
        pEmpleado.setEmail(pResultSet.getString(pIndex)); // index 3
        pIndex++;
        pEmpleado.setCargo(pResultSet.getString(pIndex)); // index 3
        return pIndex;
    }
    ...
    private static void obtenerDatos(PreparedStatement pPS, ArrayList<Empleado> pEmpleado) throws Exception {
        try (ResultSet resultSet = ComunDB.obtenerResultSet(pPS);) { // obtener el ResultSet desde la clase ComunDB
            while (resultSet.next()) { // Recorrer cada una de la fila que regresa la consulta  SELECT de la tabla Rol
                ...
                pEmpleado.add(usuario); // Agregar la entidad Rol al ArrayList de Rol
            }
    ...
    public static Empleado obtenerPorId(Empleado pUser) throws Exception {
        Empleado usuario = new Empleado();
    ...
    public static ArrayList<Empleado> obtenerTodos() throws Exception {
        ...
        try (Connection conn = ComunDB.obtenerConexion();) {// Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            String sql = obtenerSelect(new Empleado());  // Obtener la consulta SELECT de la tabla Rol
            sql += agregarOrderBy(new Empleado());  // Concatenar a la consulta SELECT de la tabla Rol el ORDER BY por Id 
            try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ComunDB
                obtenerDatos(ps, usuarios); // Llenar el ArrayList de Rol con las fila que devolvera la consulta SELECT a la tabla de Rol
                ps.close(); // Cerrar el PreparedStatement
            } catch (SQLException ex) {
    ...
    static void querySelect(Empleado pUser, ComunDB.UtilQuery pUtilQuery) throws SQLException {
    ...
    public static ArrayList<Empleado> buscar(Empleado pUser) throws Exception {
        ArrayList<Empleado> usuarios = new ArrayList();
        try (Connection conn = ComunDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            String sql = obtenerSelect(pUser); // Obtener la consulta SELECT de la tabla Rol
